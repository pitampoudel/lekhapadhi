        ...
        <header className="bg-white rounded-lg shadow-sm p-4 mb-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <BellIcon className="cursor-pointer w-6 h-6 text-gray-600 hover:text-blue-600 transition-colors" />
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
                2
              </span>
            </div>
            <div className="cursor-pointer flex items-center gap-2 bg-gray-100 hover:bg-gray-200 transition-colors rounded-full py-1 px-3">
...
function Card({ title, value, icon, trend, color = "blue" }: CardProps) {
  ...
  return (
    <div
      className={`${colorClasses[color]} p-6 rounded-lg border shadow-sm transition-transform hover:scale-105`}
    >
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-sm font-medium text-gray-700">{title}</h3>
        {icon}
      </div>
      <p className={`text-3xl font-bold mb-2 ${textColorClasses[color]}`}>
        {value}
      </p>
      {trend && (
        <p className={`text-xs ${trendColorClasses[color]}`}>{trend}</p>
      )}
    </div>
  );
}
